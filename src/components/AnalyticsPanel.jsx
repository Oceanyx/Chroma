...
export default function AnalyticsPanel({ nodes, lenses, onClose }) {
  ...
  const expandedSeeds = contentNodes.filter(n => 
    n.data.domains?.private || n.data.domains?.public || n.data.domains?.abstract
  ).length;
    ...
    <div style={{
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '20px' }}>
        <div style={{ marginBottom: '24px' }}>
          <h3 style={{ fontSize: '14px', color: '#94A3B8', marginBottom: '12px' }}>Total Seeds</h3>
          <div style={{ fontSize: '32px', fontWeight: 700, color: '#6C63FF' }}>
            {contentNodes.length}
          </div>
          <div style={{ fontSize: '12px', color: '#64748B', marginTop: '4px' }}>
            {expandedSeeds} expanded • {contentNodes.length - expandedSeeds} minimal
          </div>
        </div>

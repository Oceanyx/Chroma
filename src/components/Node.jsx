...
export default function Node({
  ...
  if (isHovered) {
    ...
    const activeLens = lenses.find(
      (l) => activeLensIds.includes(l.id) && lensIds.includes(l.id)
    );
  ...
  return (
    <div
      ...
      style={{
        ...
        border,
        boxShadow: isHovered
          ? "0 16px 48px rgba(0,0,0,0.6), 0 4px 16px rgba(108, 99, 255, 0.5)"
          : "0 6px 16px rgba(0,0,0,0.4), 0 2px 6px rgba(0,0,0,0.3)",
        borderRadius: "16px",
      ...
      <div
        style={{
          ...
          animation: hasContent
            ? "sproutGrow 0.6s cubic-bezier(0.68, -0.55, 0.265, 1.55)"
            : "none",
        }}
      >
        {seedIcon}
        ...
        >
          {["private", "public", "abstract"].map((did) => {
            ...
            return Icon ? (
              ...
              >
                <Icon size={12} color={domainColors[did]} />
              </div>
            ) : null;
          })}
          {!node.data?.domains?.private &&
